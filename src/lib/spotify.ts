...
export async function fetchPlaylistSongs(playlistId: string): Promise<PlaylistResult> {
  ...
  const MAX_PAGES = 50;
  const fields =
    "items(is_local,track(uri,type,is_local,id,name,artists(name),album(release_date,images)),item(uri,type,is_local,id,name,artists(name),album(release_date,images))),next,total";
